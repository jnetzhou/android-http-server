...
public class HttpProxyFileHandler implements HttpRequestHandler {

	public final static String TAG = "HttpProxyFileHandler";
	
	private String mWebRoot;

	public HttpProxyFileHandler(final String webRoot) {
		mWebRoot = webRoot;
	}
	...
	private StringEntity createDirListHtml(File dir, String target)
		...
		/* 上级目录 */
		if (!isSamePath(dir.getAbsolutePath(), mWebRoot)) {
	...
	private void appendRow(StringBuffer sb, File f) {
		...
		sb.append(link);
		sb.append(HttpProxyConfigs.SUFFIX_ZIP);
		sb.append("\">下载</a></span>");
		if (f.canWrite() && !hasWfsDir(f)) {
			sb.append("<span><a href=\"");
			sb.append(link);
			sb.append(HttpProxyConfigs.SUFFIX_DEL);
